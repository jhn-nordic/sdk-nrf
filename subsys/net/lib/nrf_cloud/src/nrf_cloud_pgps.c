...
#define INTERRUPTED_PNUM		5 /* prediction to interrupt at */
...
static int fail_count = FORCE_FAIL;
...
static void discard_oldest_predictions(int num)
{
	 ...
	 */
	for (pnum = npgps_num_free(); pnum < last; pnum++) {
...
static int json_send_to_cloud(cJSON *const pgps_request)
{
	...
	if (fail_count) {
		...
		LOG_INF("fail_count too large still:%d", fail_count);
	} else {
		LOG_INF("fail_count test inactive");
	}
#endif
...
static int consume_pgps_data(uint8_t pnum, const char *buf, size_t buf_len)
{
	...
	while (parsed_len < buf_len) {
		...
		case NRF_CLOUD_AGPS_UTC_PARAMETERS:
			...
			break;
		case NRF_CLOUD_AGPS_LOCATION:
			nrf_cloud_pgps_set_location_normalized(element.location->latitude,
