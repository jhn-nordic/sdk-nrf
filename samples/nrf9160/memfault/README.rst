...
* LTE metrics:
  ...
  * ``lte_time_to_connect`` - Time from the point when the device starts to search for an LTE network until the time when it gets registered with the network.
...
* Stack usage metrics:
   ...
   * ``at_cmd_unused_stack`` - The :ref:`at_cmd_readme` library's stack.
   * ``connection_poll_unused_stack``- Stack used by the cloud libraries for :ref:`lib_nrf_cloud`, :ref:`lib_aws_iot` and :ref:`lib_azure_iot_hub`.
...
* :option:`CONFIG_MEMFAULT_ROOT_CERT_STORAGE_NRF9160_MODEM` - Enabled by default when using nRF9160 DK or Thingy:91. Ensures that :c:func:`memfault_zephyr_port_install_root_certs` provisions the certificates to the modem when it is called.
...
* :option:`CONFIG_PM_PARTITION_SIZE_MEMFAULT_STORAGE` - Set the flash partition size to use when storing coredump to flash is enabled.
...
#. When the connection is established, the sample displays the captured LTE time-to-connect metric (`lte_time_to_connect``) on the terminal:
