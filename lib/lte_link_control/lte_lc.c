		       ...
		       struct lte_lc_psm_cfg *psm_cfg)
{
	...
	err = at_parser_max_params_from_str(notification,
					    NULL,
					    &resp_list,
...
static void at_handler(void *context, const char *response)
{
	...
	case LTE_LC_NOTIF_CEREG: {
		...
		if (err) {
			LOG_ERR("Notification could not be parsed, error: %d",
				err);
			return;
		}
		      ...
		      struct lte_lc_edrx_cfg *cfg)
{
	...
	if (len > (sizeof(tmp_buf) - 1)) {
		LOG_ERR("Invalid parameter length: %d", len);
		return -ENOMEM;
	}

	tmp_buf[len] = '\0';

	/* Check if index is within possible range of 16 items */
	idx = strtoul(tmp_buf, NULL, 2);
	if (idx > 16 - 1) {
		LOG_ERR("Invalid eDRX lookup index: %d", idx);
		return -EINVAL;
	}
	...
	/* The Paging Time Window is differnt for LTE-M and NB-IoT:
	 *	- LTE-M: (idx + 1) * 1.28 s
	 *	- NB-IoT (idx + 1) * 2.56 s
	 */
	idx += 1;
